...
define('_BILLING_PAGE_HEADING', 'Milagrow Humantech - Sales Home Demo for %s');
        ...
        if ($orderId) {
            ...
            if ($result = Db::getInstance()->getRow($sql)) {
                if ($result['status'] != 'paid') {
                    $billingPageHeading = sprintf(_BILLING_PAGE_HEADING, $result['category']);
...
//                    $values = array('fc' => 'module', 'module' => DemoRegistration::MODULE_NAME, 'controller' => 'paymentnotification');
                    $paymentNotificationUrl .= '/book-demo-payment-notification';
