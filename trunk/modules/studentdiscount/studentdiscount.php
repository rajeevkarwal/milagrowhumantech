...
//        $this->context->controller->addJS('/js/jquery/ui/jquery.ui.datepicker.min.js');
    ...
    public function getContent()
    {
        ...
        $this->html .= '<ul><li ' . (Tools::getValue('sd_tab') == "querieslist" || Tools::getValue('sd_tab') == NULL ||
            ...
            $url . '&sd_tab=" id = "sd_querieslist">' . $this->l('Student discount list') . '</a></li>
		<li ' . (Tools::getValue('sd_tab') == "bulk_download_queries" ? 'class="sd_admin_tab_active"' : 'class="sd_admin_tab"') . '><a style="padding:8px 8px 4px 4px;" href="' .
            $url . '&sd_tab=bulk_download_queries" id = "sd_bulk_download_queries">' . $this->l('Download Student discount list') . '</a></li></ul>';
    ...
    private function getQueriesList($fromDate, $toDate, $p = 1, $n = null)
    {
        ...
        $sql = 'select * from ' . _DB_PREFIX_ . 'student_discount as sd ' . $whereQuery . " ORDER BY created_at ".($n ? ' LIMIT ' . (int)(($p - 1) * $n) . ', ' . (int)($n) : '');
    ...
    private function getAllQueries($fromDate, $toDate)
    {
        ...
        $sql = 'select * from ' . _DB_PREFIX_ . 'student_discount as sd ' . $whereQuery. " ORDER BY created_at ";
        if ($results = Db::getInstance()->ExecuteS($sql)) {
            return $results;
        }
    ...
    private function countQueries($fromDate, $toDate)
    {
        ...
        $sql = 'select count(*) from ' . _DB_PREFIX_ . 'student_discount as sd ' . $whereQuery;
    ...
    private function bulkDownloadQueries($fromDate, $toDate)
    {
        ...
        ob_end_clean();
        header('Content-Disposition: attachment; filename=student_discount_discount_list.csv');
        header('Cache-Control: max-age=0');
        header('Content-Type: application/octet-stream');
        $content = "Serial Number,Name,Email,PhoneNumber,Interest,College,Date of Birth,City,Date \r\n";

        foreach ($queries as $key => $query) {
            $content .= $key + 1 . "," . $query['name'] . "," . $query['email'] . "," .
                $query['mobile'] . "," . $query['interest'] . "," .$query['college']. ",".$query['dob'] . "," .
    ...
    {
        $id = intval($id);
        Db::getInstance()->Execute('DELETE FROM ' . _DB_PREFIX_ . 'student_discount WHERE id_student_discount = ' . $id);
