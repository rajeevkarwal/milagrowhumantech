    ...
    private function sendMail() {
        try {
            ...
            if (!empty($customer['email'])) {
                $res = Mail::Send(
                                (int) 1, $template, $subject, $data, $customer['email'], $customer['name'], null, null, '', null, getcwd() . _MODULE_DIR_ . RentingModel::MODULE_NAME . '/', false, null
                );
            }
            ...
            if (!empty($mailto)) {
                $res = Mail::Send(
                                (int) 1, $template, $subject, $data, $mailto, $title, null, null, '', null, getcwd() . _MODULE_DIR_ . RentingModel::MODULE_NAME . '/', false, null
                );
