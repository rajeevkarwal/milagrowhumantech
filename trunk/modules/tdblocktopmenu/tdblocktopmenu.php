...
require (dirname(__FILE__) . '/tdblocktopmenu.class.php');

class TDblockTopMenu extends Module {
    ...
    public function __construct() {
        $this->name = 'tdblocktopmenu';
        $this->tab = 'front_office_features';
        $this->version = '1.1.0';
        $this->author = 'ThemesDeveloper';
        parent::__construct();
        $this->displayName = $this->l('ThemesDeveloper Top Menu Block');
    ...
    public function getContent() {
        ...
        if (Tools::isSubmit('submitBlocktopmenu')) {
            ...
            if ((!count($links_label)) && (!count($labels)))
                ...
                TDmenuTopLinks::add(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int) Shop::getContextShopID());
                $this->_html .= $this->displayConfirmation($this->l('The link has been added'));
            }
            $update_cache = true;
        } else if (Tools::isSubmit('submitBlocktopmenuRemove')) {
            $id_linksmenutop = Tools::getValue('id_linksmenutop', 0);
            TDmenuTopLinks::remove($id_linksmenutop, (int) Shop::getContextShopID());
            ...
            if (!Tools::isSubmit('link')) {
                $tmp = TDmenuTopLinks::getLinkLang($id_linksmenutop, $id_shop);
                $links_label_edit = $tmp['link'];
                $labels_edit = $tmp['label'];
                $new_window_edit = $tmp['new_window'];
            } else {
                TDmenuTopLinks::update(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int) $id_shop, (int) $id_linksmenutop, (int) $id_linksmenutop);
        ...
        $links = TDmenuTopLinks::gets($id_lang, null, (int) Shop::getContextShopID());
        foreach ($links as $link) {
            if ($link['label'] == '') {
                $link = TDmenuTopLinks::get($link['id_linksmenutop'], $default_language, (int) Shop::getContextShopID());
...
<div class="margin-form">';
        ...
        $links = TDmenuTopLinks::gets((int) $id_lang, null, (int) Shop::getContextShopID());
    ...
    private function makeMenuOption() {
        ...
        foreach ($menu_item as $item) {
            ...
            switch (substr($item, 0, strlen($values[1]))) {
                ...
                case 'LNK':
                    $link = TDmenuTopLinks::get((int) $id, (int) $id_lang, (int) $id_shop);
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = TDmenuTopLinks::get($link[0]['id_linksmenutop'], (int) $default_language, (int) Shop::getContextShopID());
                        }
            ...
            switch (substr($item, 0, strlen($value[1]))) {
                ...
                case 'LNK':
                    ...
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = TDmenuTopLinks::get($link[0]['id_linksmenutop'], $default_language, (int) Shop::getContextShopID());
                        }
        ...
        foreach ($menu_items as $item) {
            ...
            switch (substr($item, 0, strlen($value[1]))) {
                ...
                case 'CMS_CAT':
                    $category = new CMSCategory((int) $id, (int) $id_lang);
                    if (count($category)) {
                        $this->_mobile_menu .= '<li><a href="' . $category->getLink() . '">' . $category->name . '</a>';
                        $this->getCMSMenuItems($category->id);
        ...
        if (!empty($is_intersected)) {
            if (count($children)) {
                ...
                if($category->level_depth==5){
                    $this->_menu .= '<li ' . $selected . ' class="left-sub-submenu">';
                }else{
...
/*      mobile menu              */
    ...
    public function hookDisplayTop($param) {
        ...
        $smarty_cache_id = 'tdblocktopmenu-' . $this->page_name . '-' . (int) $this->context->shop->id . '-' . implode(', ', $this->user_groups) . '-' . (int) $this->context->language->id . '-' . (int) Tools::getValue('id_category') . '-' . (int) Tools::getValue('id_manufacturer') . '-' . (int) Tools::getValue('id_supplier') . '-' . (int) Tools::getValue('id_cms') . '-' . (int) Tools::getValue('id_product');
        $this->context->smarty->cache_lifetime = 31536000;
        Tools::enableCache();
        if (!$this->isCached('tdblocktopmenu.tpl', $smarty_cache_id)) {
            $this->makeMenu();
            $this->makemobileMenu();
        ...
        $html = $this->display(__FILE__, 'tdblocktopmenu.tpl', $smarty_cache_id);
    ...
    private function clearMenuCache() {
        $this->_clearCache('tdblocktopmenu.tpl');
    }
