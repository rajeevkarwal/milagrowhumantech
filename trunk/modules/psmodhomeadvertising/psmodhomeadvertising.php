...
include_once(dirname(__FILE__) . '/psmodhomeadvertisingModel.php');
include_once(dirname(__FILE__) . '/psmodhomeadvertisingClass.php');

class PSmodHomeAdvertising extends Module {
    private $_html;
    private $_display;
    
    public function __construct() {
        $this->name = 'psmodhomeadvertising';
        $this->tab = 'front_office_features';
        $this->version = '1.1.0';
        $this->author = 'Any-Themes';
        $this->need_instance = 0;
        parent::__construct();
        $this->displayName = $this->l('Any-Themes Home Advertising');
        $this->description = $this->l('Show Advertising on Home page.');
    ...
    public function install() {
        ...
        if (parent::install() && $this->registerHook('home') && $this->registerHook('header')) {
            /* Install tables */
            $res = scchomeadvertisingModel::createTables();
            return $res;
        }
        return false;
    }
    ...
    public function uninstall() {
        ...
        if (parent::uninstall()) {
            /* Deletes tables */
            $res = scchomeadvertisingModel::DropTables();
            return $res;
        }
        return false;
    }
    ...
    public function getContent() {
        ...
        elseif (Tools::isSubmit('addNewSlider') || (Tools::isSubmit('id_scchomeadvertising')))
            $this->_displayForm();
        else
            $this->_displaySlider();
    ...
    private function _insertSlider() {
        ...
        if (Tools::isSubmit('sccSubmitValue')) {
			...
			SELECT COUNT(*) 
			FROM `' . _DB_PREFIX_ . 'scchomeadvertising`');




                Db::getInstance()->Execute('
            INSERT INTO `' . _DB_PREFIX_ . 'scchomeadvertising` (`image_link`,`active`,`position`,`new_page`) 
            VALUES("' . Tools::getValue('scc_link') . '", ' . (int) Tools::getValue('scc_active_slide') . ',' . (int) $position . ',' . (int) Tools::getValue('scc_new_page') . ')');

                $id_scchomeadvertising = Db::getInstance()->Insert_ID();
                foreach ($languages as $language) {
                    $name = $_FILES['scc_image_' . $language['id_lang']]['name'];
                    $image_url = SCC_HOME_ADD_IMAGE_URL . $name;

                    $path = SCC_HOME_ADD_MODULE_IMAGE_DIR . $name;
                ...
                VALUES(' . (int) $id_scchomeadvertising . ', ' . (int) $language['id_lang'] . ', 
                "' . pSQL(Tools::getValue('scc_title_' . $language['id_lang'])) . '", 
                 "' . $image_url . '")');
                }
            } elseif (Tools::isSubmit('updateSlider')) {

                $sccsliderid = Tools::getvalue('id_scchomeadvertising');
                ...
                WHERE `id_scchomeadvertising` = ' . (int) ($sccsliderid));
                ...
                foreach ($languages as $language) {
                    ...
                    Db::getInstance()->Execute('
                            UPDATE `' . _DB_PREFIX_ . 'scchomeadvertising_lang` 
                            SET `image_title` = "' . pSQL(Tools::getValue('scc_title_' . $language['id_lang'])) . '",                    
                            `image_url` = "' . $image_url . '"
                            WHERE `id_scchomeadvertising` = ' . (int) $sccsliderid . '  AND `id_lang`= ' . (int) $language['id_lang']);
                }
            }
        }
        elseif (Tools::isSubmit('deleteSlider') AND Tools::getValue('id_scchomeadvertising')) {
            Db::getInstance()->Execute('
                DELETE FROM `' . _DB_PREFIX_ . 'scchomeadvertising`
                WHERE `id_scchomeadvertising` = ' . (int) (Tools::getValue('id_scchomeadvertising')));

            Db::getInstance()->Execute('
				DELETE FROM `' . _DB_PREFIX_ . 'scchomeadvertising_lang` 
				WHERE `id_scchomeadvertising` = ' . (int) (Tools::getValue('id_scchomeadvertising')));
        }
        if (count($errors))
    ...
    private function _postValidation() {
        ...
        elseif (Tools::isSubmit('deleteSlider') AND !Validate::isInt(Tools::getValue('id_scchomeadvertising')))
    ...
    private function _displayForm() {
        ...
        $updatevalue = NULL;
        if (Tools::isSubmit('updateSlider') AND Tools::getValue('id_scchomeadvertising'))
            $updatevalue = scchomeadvertisingModel::getSliderByID((int) Tools::getValue('id_scchomeadvertising'));
//print_r($updatevalue);
...
';
    ...
    private function _displaySlider() {
        ...
        $slider = scchomeadvertisingModel::getAllSlider();
...
<script type="text/javascript">SCCImageSlider(\'' . $this->secure_key . '\');</script>';
        ...
        foreach ($slider as $sccsliderdata):
            ...
            $this->_html .= '<tr id="tr_0_' . $sccsliderdata['id_scchomeadvertising'] . '_' . $sccsliderdata['position'] . '" ' . ($irow++ % 2 ? 'class="alt_row"' : '') . '>
                             <td>' . $sccsliderdata['id_scchomeadvertising'] . '</td>
            ...
            $this->_html .= '</td> 
                            ...
                            <td class="center pointer dragHandle">
                                    <a' . (($sccsliderdata['position'] == (sizeof($sccsliderdata) - 1) OR sizeof($sccsliderdata) == 1) ? ' style="display: none;"' : '') . ' href="' . $currentIndex . '&configure=psmodhomeadvertising&id_scchomeadvertising=' . $sccsliderdata['id_scchomeadvertising'] . '&token=' . Tools::getAdminTokenLite('AdminModules') . '">
                                    <img src="../img/admin/down.gif" alt="' . $this->l('Down') . '" title="' . $this->l('Down') . '" /></a>
                                    <a' . ($sccsliderdata['position'] == 0 ? ' style="display: none;"' : '') . ' href="' . $currentIndex . '&configure=psmodhomeadvertising&id_scchomeadvertising=' . $sccsliderdata['id_scchomeadvertising'] . '&token=' . Tools::getAdminTokenLite('AdminModules') . '">
                                    <img src="../img/admin/up.gif" alt="' . $this->l('Up') . '" title="' . $this->l('Up') . '" /></a>
                            </td>
                              <td width="10%" class="center">
                            
                                        <a href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&updateSlider&id_scchomeadvertising=' . (int) ($sccsliderdata['id_scchomeadvertising']) . '" title="' . $this->l('Edit') . '"><img src="' . _PS_ADMIN_IMG_ . 'edit.gif" alt="" /></a> 
                                        <a href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&deleteSlider&id_scchomeadvertising=' . (int) ($sccsliderdata['id_scchomeadvertising']) . '" title="' . $this->l('Delete') . '"><img src="' . _PS_ADMIN_IMG_ . 'delete.gif" alt="" /></a>
    ...
    function hookHome($params) {
        global $smarty;
            ...
            SELECT scc.`id_scchomeadvertising`, scc.`image_link`, scc.`active`, scc.`position`, scc.`new_page`,scc1.image_title, scc1.`image_url`
            ...
            return $this->display(__FILE__, 'psmodhomeadvertising.tpl');
