...
//        $this->ps_versions_compliancy = array('min' => '1.5', 'max' => '1.5');
    ...
    public function getContent()
    {
        ...
        $this->html .= '<ul><li ' . (Tools::getValue('cc_tab') == "querieslist" || Tools::getValue('cc_tab') == NULL ||
            ...
            $url . '&cc_tab=" id = "cc_querieslist">' . $this->l('Warranty queries') . '</a></li>
    ...
    private function getQueriesList($fromDate, $toDate, $p = 1, $n = null)
    {
        ...
        $sql = 'select * from ' . _DB_PREFIX_ . 'product_registration as cc ' . $whereQuery . " ORDER BY created_at DESC " . ($n ? ' LIMIT ' . (int)(($p - 1) * $n) . ', ' . (int)($n) : '');
    ...
    private function getAllQueries($fromDate, $toDate)
    {
        ...
        $sql = 'select * from ' . _DB_PREFIX_ . 'product_registration as cc ' . $whereQuery . " ORDER BY created_at DESC";
        if ($results = Db::getInstance()->ExecuteS($sql)) {
            return $results;
        }
    ...
    private function countQueries($fromDate, $toDate)
    {
        ...
        $sql = 'select count(*) from ' . _DB_PREFIX_ . 'product_registration as cc ' . $whereQuery;
    ...
    private function bulkDownloadQueries($fromDate, $toDate)
    {
        ...
        ob_end_clean();
        header('Content-Disposition: attachment; filename=Warranty queries.csv');
        header('Cache-Control: max-age=0');
        header('Content-Type: application/octet-stream');

        $content = "Serial Number,Name,Email,Mobile,Product,Product Number,Date,Store Name,Address,City,State,Date \r\n";
        foreach ($queries as $key => $query) {
            $content .= $key + 1 . "," . $query['name'] . "," . $query['email'] . ","
                . $query['mobile'] . "," . $query['product'] . "," .
                $query['product_number'] . "," . $query['date'] . ","
                . $query['store_name'] . "," . $query['address'] . "," . $query['city'] .
                "," . $query['state'] . "," . $query['created_at'] . "\r\n";
        }
        echo trim($content);
        exit;
    }
    ...
    {
        $id = intval($id);
        Db::getInstance()->Execute('DELETE FROM ' . _DB_PREFIX_ . 'product_registration WHERE id_product_registration = ' . $id);
