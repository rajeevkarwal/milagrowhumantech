...
class Partners extends Module
{
    ...
    function install()
    {
        ...
        Configuration::updateValue('TRADE_PARTNER_SALES_EMAIL','milagrowsales@milagrow.in');
        return true;
    }
    ...
    private function getQueriesList($fromDate, $toDate, $p = 1, $n = null)
    {
        ...
        $sql = 'select * from ' . _DB_PREFIX_ . 'partners as cc ' . $whereQuery . " ORDER BY created_at DESC " . ($n ? ' LIMIT ' . (int)(($p - 1) * $n) . ', ' . (int)($n) : '');
    ...
    private function getAllQueries($fromDate, $toDate)
    {
        ...
        $sql = 'select * from ' . _DB_PREFIX_ . 'partners as cc ' . $whereQuery . " ORDER BY created_at DESC";
        if ($results = Db::getInstance()->ExecuteS($sql)) {
            return $results;
        }
    ...
    private function countQueries($fromDate, $toDate)
    {
        ...
        $sql = 'select count(*) from ' . _DB_PREFIX_ . 'partners as cc ' . $whereQuery;
...
//                    `created_at` datetime NOT NULL,
        ...
        ob_end_clean();
        header('Content-Disposition: attachment; filename=partners queries.csv');
        header('Cache-Control: max-age=0');
        header('Content-Type: application/octet-stream');
        $content = "Serial Number,Name,Email,Phone,Product,State,City,Purpose,Website,Turnover,Message,Date \r\n";
        foreach ($queries as $key => $query) {
            $content .= $key + 1 . "," . $query['name_of_company'] . "," . $query['email'] . "," .
                $query['contact_number'] . "," . $query['product'] . "," . $query['state'] . "," .
                $query['city'] . "," . $query['purpose'] . ","
                . $query['website'] . "," . $query['turnover'] . "," . $query['message'] . "," . $query['created_at'] . "\r\n";
        }
        echo trim($content);
        exit;
    }
    ...
    {
        $id = intval($id);
        Db::getInstance()->Execute('DELETE FROM ' . _DB_PREFIX_ . 'partners WHERE id_partners = ' . $id);
