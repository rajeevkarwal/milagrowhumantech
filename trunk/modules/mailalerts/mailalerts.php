...
class MailAlerts extends Module
{
    ...
    {
        /* ----------------- we have return the code to where it call from as not to send the email
         ----------------- we have added this functionality to payment module and added this mail functionality their itself
         --------------------*/
        ...
        $order = $params['order'];
        if ($order->payment == 'COD' || $order->payment == 'cod' || $order->payment == 'EMI' || $order->payment == 'emi')
            return;
        ...
        $order_status = (int)$params['orderStatus']->paid;
...
//                );
            ...
            //error while transaction so send error mail
            $template = 'new_order_fail';
            if (file_exists(dirname(__FILE__) . '/mails/' . $iso . '/' . $template . '.html')
                ...
                Mail::Send(
                    $id_lang,
                    $template,
                    sprintf(Mail::l('New Order Payment Not Received - #%06d', $id_lang), $order->id),
                    $template_vars,
                    $logisticEmailId, //toemail id
                    null,
                    null,
...
//                    $id_lang,
//                    $template,
//                    sprintf(Mail::l('New order Payment Failure - #%06d', $id_lang), $order->id),
//                    $template_vars,
//                    "hmalhotra@greenapplesolutions.com",
//                    null,
//                    null,
