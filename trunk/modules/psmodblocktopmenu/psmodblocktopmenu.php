...
require (dirname(__FILE__) . '/psmodblocktopmenu.class.php');

class PsModBlockTopMenu extends Module {

    private $_menu = '';
    private $_mobile_menu = '';
    ...
    public function __construct() {
        $this->name = 'psmodblocktopmenu';
        $this->tab = 'front_office_features';
        $this->version = '1.1.0';
        $this->author = 'Any-Themes';
        parent::__construct();
        $this->displayName = $this->l('Any-Themes Superfish Menu Block');
        $this->description = $this->l(' Add categories and custom links.');
    }
    ...
    public function getContent() {
        ...
        if (Tools::isSubmit('submitBlocktopmenu')) {
            ...
            if ((!count($links_label)) && (!count($labels)))
                ...
                sccMenuTopLinks::add(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int) Shop::getContextShopID());
                $this->_html .= $this->displayConfirmation($this->l('The link has been added'));
            }
            $update_cache = true;
        } else if (Tools::isSubmit('submitBlocktopmenuRemove')) {
            $id_linksmenutop = Tools::getValue('id_linksmenutop', 0);
            sccMenuTopLinks::remove($id_linksmenutop, (int) Shop::getContextShopID());
            ...
            if (!Tools::isSubmit('link')) {
                $tmp = sccMenuTopLinks::getLinkLang($id_linksmenutop, $id_shop);
                $links_label_edit = $tmp['link'];
                $labels_edit = $tmp['label'];
                $new_window_edit = $tmp['new_window'];
            } else {
                sccMenuTopLinks::update(Tools::getValue('link'), Tools::getValue('label'), Tools::getValue('new_window', 0), (int) $id_shop, (int) $id_linksmenutop, (int) $id_linksmenutop);
        ...
        $this->_html .= '<optgroup label="' . $this->l('Custom Menu Links') . '">';
        $links = sccMenuTopLinks::gets($id_lang, null, (int) Shop::getContextShopID());
        foreach ($links as $link) {
            if ($link['label'] == '') {
                $link = sccMenuTopLinks::get($link['id_linksmenutop'], $default_language, (int) Shop::getContextShopID());
        ...
        $this->_html .= '
		<fieldset>
			<legend><img src="../img/admin/add.gif" alt="" title="" />' . $this->l('Add Custom Menu') . '</legend>
			<form action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) . '" method="post" id="form">
...
<div class="margin-form">';
        ...
        $links = sccMenuTopLinks::gets((int) $id_lang, null, (int) Shop::getContextShopID());
        ...
        $this->_html .= '
		<fieldset>
			<legend><img src="../img/admin/details.gif" alt="" title="" />' . $this->l('List Custom Menu') . '</legend>
    ...
    private function makeMenuOption() {
        ...
        foreach ($menu_item as $item) {
            ...
            switch (substr($item, 0, strlen($values[1]))) {
                ...
                case 'LNK':
                    $link = sccMenuTopLinks::get((int) $id, (int) $id_lang, (int) $id_shop);
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = sccMenuTopLinks::get($link[0]['id_linksmenutop'], (int) $default_language, (int) Shop::getContextShopID());
                        }
            ...
            switch (substr($item, 0, strlen($value[1]))) {
                ...
                case 'LNK':
                    ...
                    if (count($link)) {
                        if (!isset($link[0]['label']) || ($link[0]['label'] == '')) {
                            $default_language = Configuration::get('PS_LANG_DEFAULT');
                            $link = sccMenuTopLinks::get($link[0]['id_linksmenutop'], $default_language, (int) Shop::getContextShopID());
                        }
 ...
 private function makemobileMenu() {
        ...
        foreach ($menu_items as $item) {
            ...
            switch (substr($item, 0, strlen($value[1]))) {
                ...
                case 'PRD':
                    ...
                    $product = new Product((int) $id, true, (int) $id_lang);
                    if (!is_null($product->id))
                        $this->_mobile_menu .= '<li' . $selected . '><a href="' . $product->getLink() . '">' . $product->name . '</a></li>' . PHP_EOL;
                    break;
                ...
                case 'CMS':
                    ...
                    $cms = CMS::getLinks((int) $id_lang, array($id));
                    if (count($cms))
                        $this->_mobile_menu .= '<li' . $selected . '><a href="' . $cms[0]['link'] . '">' . $cms[0]['meta_title'] . '</a></li>' . PHP_EOL;
                    break;
                ...
                case 'CMS_CAT':
                    ...
                    if (count($category)) {
                            ...
                            $categories = $this->getCMSCategories(false, (int) $category->id, (int) $id_lang);
                            
                            $pages = $this->getCMSPages((int)$category->id);
                            
                            
                        if (count($categories) || count($pages)) {
                            $this->_mobile_menu .= '<li class="parent">';     
                                $this->_mobile_menu .=  '<div class="menuopen" style="opacity: 1;"></div>';
                            
                        }else{
                             $this->_mobile_menu .= '<li>'; 
                        }
  
                        $this->_mobile_menu .='<a href="' . $category->getLink() . '">' . $category->name . '</a>';
           

                        
                        
                        $this->getCMSMobileMenuItems($category->id);
                        


                        $this->_mobile_menu .= '</li>' . PHP_EOL;
                ...
                case 'MAN':
                    ...
                    if (!is_null($manufacturer->id)) {
                        if (intval(Configuration::get('PS_REWRITING_SETTINGS')))
                            ...
                            $manufacturer->link_rewrite = 0;
                        $link = new Link;
                        $this->_mobile_menu .= '<li' . $selected . '><a href="' . $link->getManufacturerLink((int) $id, $manufacturer->link_rewrite) . '">' . $manufacturer->name . '</a></li>' . PHP_EOL;
                    }
                    break;
                ...
                case 'SUP':
                    ...
                    if (!is_null($supplier->id)) {
                        $link = new Link;
                        $this->_mobile_menu .= '<li' . $selected . '><a href="' . $link->getSupplierLink((int) $id, $supplier->link_rewrite) . '">' . $supplier->name . '</a></li>' . PHP_EOL;
                    }
                    break;
                ...
                case 'SHOP':
                    ...
                    if (Validate::isLoadedObject($shop)) {
                        $link = new Link;
                        $this->_mobile_menu .= '<li' . $selected . '><a href="' . $shop->getBaseURL() . '">' . $shop->name . '</a></li>' . PHP_EOL;
                    }
                    break;
                case 'LNK':
                    $link = sccMenuTopLinks::get((int) $id, (int) $id_lang, (int) $id_shop);
                    if (count($link)) {
                        ...
                        $this->_mobile_menu .= '<li><a href="' . $link[0]['link'] . '"' . (($link[0]['new_window']) ? ' target="_blank"' : '') . '>' . $link[0]['label'] . '</a></li>' . PHP_EOL;
                    }
                    break;
            }
        }
    ...
    private function getCategory($id_category, $id_lang = false, $id_shop = false) {
        ...
        if (!empty($is_intersected)) {
            ...
            $this->_menu .= '<a href="' . $category_link . '"><span>' . $category->name . '</span></a>';
 ...
 private function getCMSMobileMenuItems($parent, $depth = 1, $id_lang = false) {
        $id_lang = $id_lang ? (int) $id_lang : (int) Context::getContext()->language->id;
      ...
      //  print_r($pages); 
        ...
        if (count($categories) || count($pages)) {
            ...
            $this->_mobile_menu .= '<ul>';

            foreach ($categories as $category) {
              $this->_mobile_menu .=  '<li>';
               $this->_mobile_menu .= '<a href="#">' . $category['name'] . '</a>';
                $this->getCMSMobileMenuItems($category['id_cms_category'], (int) $depth + 1);
                $this->_mobile_menu .= '</li>';
            }
            ...
            foreach ($pages as $page) {
                ...
                $selected = ($this->page_name == 'cms' && ((int) Tools::getValue('id_cms') == $page['id_cms'])) ? ' class="sfHoverForce"' : '';
                $this->_mobile_menu .=  '<li ' . $selected . '>';
                $this->_mobile_menu .=  '<a href="' . $links[0]['link'] . '">' . $cms->meta_title . '</a>';
               $this->_mobile_menu .=  '</li>';
            }
...
/*      mobile menu              */
    ...
    private function getmobleCategory($id_category, $id_lang = false, $id_shop = false) {
        $id_lang = $id_lang ? (int) $id_lang : (int) Context::getContext()->language->id;
        $category = new Category((int) $id_category, (int) $id_lang);
        ...
        if (!empty($is_intersected)) {
            if (count($children)) {
                $this->_mobile_menu .= '<li ' . $selected . ' class="parent">';
            } else {
                $this->_mobile_menu .= '<li ' . $selected . '>';
            }
          $parent = 'parent';
            ...
            $this->_mobile_menu .= '<a href="' . $category_link . '">' . $category->name . '</a>';
            ...
            if (count($children)) {
                ...
                foreach ($children as $child)
                    $this->getmobleCategory((int) $child['id_category'], (int) $id_lang, (int) $child['id_shop']);
    ...
    public function hookDisplayTop($param) {
        ...
        $smarty_cache_id = 'sccblocktopmenu-' . $this->page_name . '-' . (int) $this->context->shop->id . '-' . implode(', ', $this->user_groups) . '-' . (int) $this->context->language->id . '-' . (int) Tools::getValue('id_category') . '-' . (int) Tools::getValue('id_manufacturer') . '-' . (int) Tools::getValue('id_supplier') . '-' . (int) Tools::getValue('id_cms') . '-' . (int) Tools::getValue('id_product');
        $this->context->smarty->cache_lifetime = 31536000;
        Tools::enableCache();
        if (!$this->isCached('psmodblocktopmenu.tpl', $smarty_cache_id)) {
            $this->makeMenu();
            $this->makemobileMenu();
            $this->smarty->assign('MobileMENU', $this->_mobile_menu);
        ...
        $this->context->controller->addCSS($this->_path . 'css/superfish-vertical.css');
        $this->context->controller->addCSS($this->_path . 'css/superfish.css');
        
        $html = $this->display(__FILE__, 'psmodblocktopmenu.tpl', $smarty_cache_id);
    ...
    private function clearMenuCache() {
        $this->_clearCache('psmodblocktopmenu.tpl');
    }
