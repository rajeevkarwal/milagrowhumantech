...
include_once(dirname(__FILE__) . '/tdhomeadvertising_class.php');
...
class TDhomeAdvertising extends Module {
    ...
    public function __construct() {
        ...
        parent::__construct();
        $this->displayName = $this->l('ThemesDeveloper  Home Advertising');
        $this->description = $this->l('Home page Advertising by ThemesDeveloper.');
    ...
    public function install() {
        ...
        if (parent::install() && $this->registerHook('home') && $this->registerHook('header')) {
            /* Install tables */
            $res = tdhomeadvertisingModel::createTables();
            return $res;
        }
        return false;
    }
    ...
    public function uninstall() {
        ...
        if (parent::uninstall()) {
            /* Deletes tables */
            $res = tdhomeadvertisingModel::DropTables();
            return $res;
        }
        return false;
    }
    ...
    private function _insertSlider() {
        ...
        if (Tools::isSubmit('tdSubmitValue')) {
            ...
            INSERT INTO `' . _DB_PREFIX_ . 'tdhomeadvertising` (`image_link`,`active`,`position`,`new_page`) 
            VALUES("' . Tools::getValue('td_link') . '", ' . (int) Tools::getValue('td_active_slide') . ',' . (int) $position . ',' . (int) Tools::getValue('td_new_page') . ')');
                ...
                foreach ($languages as $language) {
                    ...
                    $image_url = TD_HOME_ADVERTISING_IMAGE_URL . $name;

                    $path = TD_HOME_ADVERTISING_MODULE_IMAGE_DIR . $name;
...
//print_r($updatevalue);
        ...
        $this->_html .= '
		<fieldset>
			<legend><img src="' . _PS_BASE_URL_ . __PS_BASE_URI__ . 'modules/' . $this->name . '/logo.gif" alt="" /> ' . $this->l('td Home Advertising') . '</legend>
			';
...
<script type="text/javascript">TDhomeAdvertising(\''. $this->secure_key . '\');</script>';
        ...
        foreach ($slider as $tdsliderdata):
            ...
            $this->_html .= '<tr id="tr_0_' . $tdsliderdata['id_tdhomeadvertising'] . '_' . $tdsliderdata['position'] . '" ' . ($irow++ % 2 ? 'class="alt_row"' : '') . '>
                             <td>' . $tdsliderdata['id_tdhomeadvertising'] . '</td>
            ...
            $this->_html .= '</td> 
                            ...
                            <td class="center pointer dragHandle">
                                    <a' . (($tdsliderdata['position'] == (sizeof($tdsliderdata) - 1) OR sizeof($tdsliderdata) == 1) ? ' style="display: none;"' : '') . ' href="' . $currentIndex . '&configure=tdhomeadvertising&id_tdhomeadvertising=' . $tdsliderdata['id_tdhomeadvertising'] . '&token=' . Tools::getAdminTokenLite('AdminModules') . '">
                                    <img src="../img/admin/down.gif" alt="' . $this->l('Down') . '" title="' . $this->l('Down') . '" /></a>
                                    <a' . ($tdsliderdata['position'] == 0 ? ' style="display: none;"' : '') . ' href="' . $currentIndex . '&configure=tdhomeadvertising&id_tdhomeadvertising=' . $tdsliderdata['id_tdhomeadvertising'] . '&token=' . Tools::getAdminTokenLite('AdminModules') . '">
                                    <img src="../img/admin/up.gif" alt="' . $this->l('Up') . '" title="' . $this->l('Up') . '" /></a>
                            </td>
                              <td width="10%" class="center">
                            
                                        <a href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&updateSlider&id_tdhomeadvertising=' . (int) ($tdsliderdata['id_tdhomeadvertising']) . '" title="' . $this->l('Edit') . '"><img src="' . _PS_ADMIN_IMG_ . 'edit.gif" alt="" /></a> 
                                        <a href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&deleteSlider&id_tdhomeadvertising=' . (int) ($tdsliderdata['id_tdhomeadvertising']) . '" title="' . $this->l('Delete') . '"><img src="' . _PS_ADMIN_IMG_ . 'delete.gif" alt="" /></a>
