...
//					' . (int)Tools::ps_round($max_price[$currency['id_currency']] * (100 + $max_tax_rate) / 100, 0) . ')';
    ...
    public function getProducts($selected_filters, &$products, &$nb_products, &$p, &$n, &$pages_nb, &$start, &$stop, &$range)
    {
        ...
        foreach($products as &$product){
            $productObject= new Product($product['id_product'], true, $this->context->language->id, $this->context->shop->id);
            $product['allow_oosp']=ProductCore::isAvailableWhenOutOfStock($productObject->out_of_stock);
            $product['price'] = Tools::displayPrice(Product::getPriceStatic((int)$product['id_product'], true), new Currency($cookie->id_currency));
        }
