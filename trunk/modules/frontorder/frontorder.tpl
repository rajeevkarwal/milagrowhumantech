...
{if $order->total_wrapping > 0}
    <tr class="item">
        <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
            {l s='(Add) Gift Wrapping Cost: '} <span
...
{if $order->payment eq 'COD' || $order->payment eq 'cod'}
    ...
    {if $down_payment>0}
        <tr class="item">
            <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
                {l s='Non Refundable Advance for COD Order Processing: '} <span
                        class="price">{displayWtPriceWithCurrency price=750 currency=$currency}</span>
            </td>
        </tr>
    {/if}
    {if $advance_amount>0}
        <tr class="item">
            <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
                {l s='Additional Advance Amount collected (As balance cash to be collected can not exceed Rs 20,000/- as per company policy): '}
                <span
                        class="price">{displayWtPriceWithCurrency price=$advance_amount currency=$currency}</span>
            </td>
        </tr>
    {/if}

    {if $down_payment>=0 and $down_payment != $order->total_paid}
        <tr class="item">
            <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
                {l s='Balance to be paid at the time of delivery: '}
                <span
    ...
    {else}
        <tr class="item">
            <td colspan="{if $return_allowed || $order->hasProductReturned()}{if $order->hasProductReturned() && $return_allowed}7{else}6{/if}{else}5{/if}">
                {l s='Balance paid at the time of delivery: '}
                <span
                        class="price">{displayPrice currency=$order->id_currency price=($order->total_paid-750-$advance_amount)}</span>
            </td>
        </tr>
    {/if}
{elseif $order->payment eq 'Citibank EMI' || $order->payment eq 'emi'}
    ...
    <tr class="item">
        <td colspan=5>(Add) EMI File Charge ({$EMI_TAX_RATE}%): <span
                    class="price-wrapping"> {displayPrice currency=$order->id_currency price=$EMI_PROCESSING_FEES}</span>
        </td>
    </tr>
    <tr class="item">
        <td colspan=5>(Add) Service Tax 12.36%
            of  {displayPrice currency=$order->id_currency price=$EMI_PROCESSING_FEES}: <span
                    class="price-wrapping"> {displayPrice currency=$order->id_currency price=$EMI_SERVICE_CHARGE_FEES}</span>
        </td>
    </tr>
    <tr class="item">
        <td colspan=5>Total Invoice Value: <span
