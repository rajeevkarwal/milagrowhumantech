...
class ProductComment extends ObjectModel
{
     ...
     */
    public static function getByProduct($id_product, $p = 1, $n = null, $id_customer = null,$orderBy='date')
    {
        ...
        else if($orderBy=='useful')
            $orderBy='total_useful DESC,pc.`date_add` DESC';
        else if($orderBy=='grade')
            $orderBy='pc.`grade` DESC, `date_add` DESC';
		...
		WHERE pc.`id_product` = ' . (int)($id_product) . ($validate == '1' ? ' AND pc.`validate` = 1' : '') . ' AND (pc.title!=\'\' and pc.title is NOT NULL and pc.title!=\'only rating\') AND (pc.content!=\'\' and pc.content is NOT NULL and pc.title!=\'only rating\')
		ORDER BY '.$orderBy.'
...
//custom function for count grades and average grades used for ratings
    public static function getAverageGradeAndCountRatings($id_product)
    {
        $validate = Configuration::get('PRODUCT_COMMENTS_MODERATE');

        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
		SELECT COUNT(pc.`grade`) AS totalRating, AVG(pc.`grade`) as averageGrade
		FROM `' . _DB_PREFIX_ . 'product_comment` pc
		WHERE pc.`id_product` = ' . (int)$id_product . '
		AND pc.`deleted` = 0');
    }

    //custom function added to count reveiws
    public static function countReviews($id_product)
    {
        $validate = Configuration::get('PRODUCT_COMMENTS_MODERATE');

        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
		SELECT COUNT(pc.`id_product_comment`) AS total
		FROM `' . _DB_PREFIX_ . 'product_comment` pc
		WHERE pc.`id_product` = ' . (int)$id_product . '
		AND (pc.title!=\'\' and pc.title is NOT NULL and pc.title!=\'only rating\') AND (pc.content!=\'\' and pc.content is NOT NULL and pc.title!=\'only rating\') AND pc.`deleted` = 0' .
            ($validate == '1' ? ' AND pc.`validate` = 1' : ''));
    }

    //custom function to get grade for a product

    public static function getCommentForProduct($id_product, $id_customer)
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('SELECT * from ' . _DB_PREFIX_ . 'product_comment where id_product=\'' . $id_product . '\' and id_customer=\'' . $id_customer.'\' and deleted=0');
    }
