...
">
    ...
    public function hookproductOutOfStock($params)
    {
        ...
        $totalRating = 0;
        $totalReviews = 0;
        $ratings = ProductComment::getAverageGradeAndCountRatings((int)Tools::getValue('id_product'));
        $reviews = ProductComment::countReviews((int)Tools::getValue('id_product'));
        $newReview = true;
        ...
        if ($ratings) {
            $averageRating = round($ratings['averageGrade'], 2);
...
//		$average = ProductComment::getAverageGrade((int)Tools::getValue('id_product'));
        ...
        $this->context->smarty->assign(array(
            ...
            'logged' => (int)$this->context->customer->isLogged(),
            'allow_guests' => (int)Configuration::get('PRODUCT_COMMENTS_ALLOW_GUESTS'),
...
//            'averageTotal' => (int)$average['grade'],
    ...
    public function hookProductTabContent($params)
    {
        ...
        $this->context->smarty->assign(array(
            ...
            'action_url' => '',
            'comments' => ProductComment::getByProduct((int)Tools::getValue('id_product'), 1, 5, $this->context->cookie->id_customer,'grade'),
            'criterions' => ProductCommentCriterion::getByProduct((int)Tools::getValue('id_product'), $this->context->language->id),
            'averages' => $averages,
    ...
    public function hookHeader()
    {
        ...
        $this->context->controller->addCSS($this->_path . 'jquery.rating.css', 'all');
    }
    ...
    public function addorUpdateGradeForTheGivenProduct($id_product, $grade)
    {

        $context = Context::getContext();
        $commentExist = ProductComment::getCommentForProduct($id_product, $context->cookie->id_customer);
        if ($commentExist) {
            $id_product_comment = $commentExist['id_product_comment'];
            Db::getInstance()->update('product_comment', array('grade' => $grade, 'date_add' => date('Y-m-d H:i:s')), "id_product_comment=$id_product_comment");
        ...
        if (!$customer_name)
            $customer_name = pSQL($context->customer->firstname . ' ' . $context->customer->lastname);
        $insertData = array('content' => '', 'id_product' => (int)$id_product, 'id_customer' => (int)$context->cookie->id_customer,
            'id_guest' => $context->cookie->id_customer, 'customer_name' => $customer_name, 'title' => '', 'grade' => $grade, 'validate' => 0, 'date_add' => date('Y-m-d H:i:s'));
        Db::getInstance()->insert('product_comment', $insertData);

        return true;
    }

    public function getGradeForTheGivenProduct($id_product)
    {
        $context = Context::getContext();
        $comment = ProductComment::getCommentForProduct($id_product, $context->cookie->id_customer);
        if ($comment)
