...
class WishList extends ObjectModel
{
    ...
    public static function isExistsByNameForUser($name)
    {
        ...
        elseif (Shop::getContextShopGroupID())
            $shop_restriction = 'AND id_shop_group = ' . (int)Shop::getContextShopGroupID(); else
            $shop_restriction = '';
    ...
    public static function getProductByIdCustomer($id_wishlist, $id_customer, $id_lang, $id_product = null, $quantity = false)
    {
		...
		SELECT wp.`id_product`, wp.`quantity`, p.`quantity` AS product_quantity,AVG(pc.`grade`) as rating,Count(DISTINCT(pc.`id_product_comment`)) as reviews, pl.`name`, wp.`id_product_attribute`, wp.`priority`, pl.link_rewrite, cl.link_rewrite AS category_rewrite
    ...
    public static function getAllProductsAddedToWishlistForCustomer($id_customer, $id_lang, $id_product = null, $quantity = false, $p = 1, $n = null)
    {
        ...
        if ($n != null && $n <= 0)
            $n = 10;
        $products = Db::getInstance()->executeS('
		SELECT wp.`id_product`, wp.`quantity`, p.on_sale,p.show_price,AVG(pc.`grade`) as rating,Count(DISTINCT(pc.`id_product_comment`)) as reviews,p.condition, pl.`description_short`,p.`quantity` AS product_quantity, pl.`name`, wp.`id_product_attribute`, wp.`priority`, pl.link_rewrite, cl.link_rewrite AS category_rewrite
		FROM `' . _DB_PREFIX_ . 'wishlist_product` wp
		LEFT JOIN `' . _DB_PREFIX_ . 'product` p ON p.`id_product` = wp.`id_product`
		' . Shop::addSqlAssociation('product', 'p') . '
		LEFT JOIN `' . _DB_PREFIX_ . 'product_lang` pl ON pl.`id_product` = wp.`id_product`' . Shop::addSqlRestrictionOnLang('pl') . '
		...
		AND pl.`id_lang` = ' . (int)($id_lang) .
            ...
            ($quantity == true ? ' AND wp.`quantity` != 0' : '') . '
            AND (pc.`deleted` = 0 OR pc.`deleted`is NULL)
		GROUP BY p.id_product, wp.id_product_attribute ' . ($n ? 'LIMIT ' . (int)(($p - 1) * $n) . ', ' . (int)($n) : ''));
        if (empty($products) === true OR !sizeof($products))
            return array();
        $products = Product::getProductsProperties($id_lang, $products);
        for ($i = 0; $i < sizeof($products); ++$i) {
            $cover = Product::getCover((int)$products[$i]['id_product']);
            $products[$i]['id_image'] = Tools::htmlentitiesUTF8(Product::defineProductImage(array('id_image' => $cover['id_image'], 'id_product' => $products[$i]['id_product']), $id_lang));
            if (isset($products[$i]['id_product_attribute']) AND
                Validate::isUnsignedInt($products[$i]['id_product_attribute'])
            ) {
                $result = Db::getInstance()->executeS('
    ...
    public static function countWishListItems($id_customer)
    {
        $countWishListItems = 'Select count(1) from ' . _DB_PREFIX_ . 'wishlist_product wp left join ' . _DB_PREFIX_ . 'wishlist w on wp.id_wishlist=w.id_wishlist where w.id_customer=' . $id_customer;
        return Db::getInstance()->getValue($countWishListItems);
    }
    ...
    {
        $sqlQuery = 'Select * from ' . _DB_PREFIX_ . 'wishlist where id_wishlist=' . $id_wishlist;
