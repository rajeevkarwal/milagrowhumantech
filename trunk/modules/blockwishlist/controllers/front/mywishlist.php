...
class BlockWishListMyWishListModuleFrontController extends ModuleFrontController
{
    ...
    public function assign()
    {
        ...
        if ($this->context->customer->isLogged()) {
            ...
            $nextPage = 0;
            $pageCount = 5;
            $products = WishList::getAllProductsAddedToWishlistForCustomer($customerId, $langID, null, false, $page, $pageCount);
            $totalWishListItems = WishList::countWishListItems($customerId);
            if (($page * $pageCount) < (int)$totalWishListItems) {
                $nextPage = $page + 1;
            }
            if ($page > 1)
                $previousPage = $page - 1;
            $wishlist = WishList::getWishList($this->context->cookie->id_wishlist);
...
//			$this->context->smarty->assign('nbProducts', WishList::getInfosByIdCustomer($this->context->customer->id));
    ...
    private function getWishListForCurrentUser($customerId)
    {
