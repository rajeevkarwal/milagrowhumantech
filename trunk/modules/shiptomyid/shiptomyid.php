...
class Shiptomyid extends Module {
    ...
    public function install() {
        if (!parent::install() || !$this->registerHook('displayHeader') || !$this->registerHook('actionValidateOrder') || !$this->registerHook('actionObjectOrderHistoryAddAfter') || !$this->registerHook('actionObjectOrderUpdateAfter') || !$this->registerHook('displayAdminOrder') || !$this->registerHook('displayProductButtons') || !$this->installSQL() || !$this->installExternalOrderState())
            return false;
        ...
        Configuration::updateValue('GIFTNOW_ENABLE', 0);
        Configuration::updateValue('GIFTNOW_POPUP_URL', Configuration::get('PS_SSL_ENABLED') ? 'https://hotfix-app.ship2myid.com/ship2myid/shopping_cart_popup/ship2myid_gift_now.jsp?platform=prestashop' : 'http://hotfix-app.ship2myid.com/ship2myid/shopping_cart_popup/ship2myid_gift_now.jsp?platform=prestashop');
        Configuration::updateValue('GIFTNOW_FOR_CATE', 0);
        Configuration::updateValue('GIFTNOW_FOR_PROD', 0);
        Configuration::updateValue('GIFTNOW_FOR_ATTR', 0);
        Configuration::updateValue('GIFTNOW_CATRGORIES', '');
        Configuration::updateValue('GIFTNOW_PRODUCTS', '');
        Configuration::updateValue('GIFTNOW_ATTRIBUTE', '');
        Configuration::updateValue('GIFTNOW_MSG', '');
        return true;
    }
    ...
    public function isAvailable() {
        ...
        if (!Configuration::get('SHIPTOMYID_ENABLE') || !Configuration::get('SHIPTOMYID_POPUP_URL') || !Configuration::get('SHIPTOMYID_USERNAME') || !Configuration::get('SHIPTOMYID_PASSWORD'))
            return false;
    ...
    public function hookDisplayProductButtons() {
        if (!$this->isAvailable() || !Configuration::get('GIFTNOW_ENABLE'))
            return;
        $product_name = '';
        $product_price = '';
        $product_image = '';
        $id_product = (int) Tools::getValue('id_product');
        if (!empty($id_product) && $id_product > 0 && $this->isProductAvailabbleForGiftNow($id_product)) {
            $id_product = (int) Tools::getValue('id_product');
            $product = new Product($id_product, true, $this->context->language->id, $this->context->shop->id);
            $p = round($product->price, 2);
            $sp = round($product->specificPrice, 2);
            if ($sp > 0 && $sp < $p) {
	$p = $sp;
            }
            ...
            $product_image = 'http://' . $link->getImageLink($product->link_rewrite, $image['id_image'], 'home_default');
        } else {
            return;
        }
        $marketplace_details = '';
        if (isset($this->context->ship2myid->marketplace) && is_array($this->context->ship2myid->marketplace))
            ...
            $marketplace_details = $this->api->getMarketplaceDetails();


        $this->context->ship2myid = isset($this->context->ship2myid) ? $this->context->ship2myid : new stdClass();
        $this->context->ship2myid->marketplace = $marketplace_details;
        ...
        $key = md5(Configuration::get('SHIPTOMYID_USERNAME'));
        $this->context->controller->addCSS(__PS_BASE_URI__ . 'modules/' . $this->name . '/views/css/shiptomyid-fancybox.css', 'all');
        $this->context->controller->addCSS(__PS_BASE_URI__ . 'modules/' . $this->name . '/views/css/giftnow.css', 'all');
        $this->context->controller->addjqueryPlugin('fancybox');
        $this->context->smarty->assign(array(
            'popup_url' => Configuration::get('GIFTNOW_POPUP_URL') . '&prod_name=' . $product_name . '&prod_img=' . $product_image . '&prod_price=' . $product_price,
            'popup_width' => Configuration::get('SHIPTOMYID_POPUP_WIDTH'),
            'popup_height' => Configuration::get('SHIPTOMYID_POPUP_HEIGHT'),
            'callback_url' => Tools::getHttpHost(true) . __PS_BASE_URI__ . 'modules/' . $this->name . '/postdata.php?key=' . $key . '&giftnow=1',
            'marketplace_name' => isset($marketplace_name) && !empty($marketplace_name) ? $marketplace_name : '',
            'marketplace_image' => isset($marketplace_name) && !empty($marketplace_image) ? $marketplace_image : '',
            'giftnow_msg' => Configuration::get('GIFTNOW_MSG')
        ));
        return $this->display(__FILE__, 'giftnow-popup.tpl');
    }

    public function isProductAvailabbleForGiftNow($id_product) {
        //$giftnow_for = explode(',', Configuration::get('GIFTNOW_FOR'));
        if ($id_product) {
            $giftnow_for_cat = Configuration::get('GIFTNOW_FOR_CATE');
            $giftnow_for_prod = Configuration::get('GIFTNOW_FOR_PROD');
            $giftnow_for_attr = Configuration::get('GIFTNOW_FOR_ATTR');

            $giftnow_cat = Configuration::get('GIFTNOW_CATRGORIES');
            $giftnow_prod = Configuration::get('GIFTNOW_PRODUCTS');
            $giftnow_attr = Configuration::get('GIFTNOW_ATTRIBUTE');
            if ($giftnow_for_cat && !empty($giftnow_cat)) {
	$giftnow_cat = explode(',', $giftnow_cat);
	$_pcats = Product::getProductCategories($id_product);
	foreach ($giftnow_cat as $_gcat) {
	    //echo 'cat';
	    if (in_array($_gcat, $_pcats))
	        return true;
	}
            }
            if ($giftnow_for_prod && !empty($giftnow_prod)) {
	$giftnow_prod = explode(',', $giftnow_prod);
	//echo 'prod';
	if (in_array($id_product, $giftnow_prod))
	    return true;
            }
            if ($giftnow_for_attr && !empty($giftnow_attr)) {
	list($_fname, $_fid) = explode(':', $giftnow_attr);
	if ($_fid) {
	    $_pfs = Product::getFeaturesStatic($id_product);
	    foreach ($_pfs as $_pf) {
	        //echo 'attr';
	        if ($_pf['id_feature'] == $_fid)
	            return true;
	    }
	}
            }
	...
	foreach ($ids_address as $id_address)
        ...
        if (isset($_SESSION['giftnow']) && !empty($_SESSION['giftnow'])) {
            //$_SESSION['giftnow']['count'] = 1;
            if (!isset($_SESSION['giftnow']['count'])) {
	$_SESSION['giftnow']['count'] = 1;
	echo '<script language="javascript" type="text/javascript">top.location.href = "' . $this->context->link->getModuleLink('shiptomyid', 'front') . '" ;</script>';
            } elseif ($_SESSION['giftnow']['count'] < 2) {
	echo '<script language="javascript" type="text/javascript">top.location.href = "' . $this->context->link->getModuleLink('shiptomyid', 'front') . '" ;</script>';
	$_SESSION['giftnow']['count'] += 1;
            } else {
	$_SESSION['giftnow']['count'] += 1;
...
//			'callback_url' => Tools::getShopDomain(true).__PS_BASE_URI__.'modules/'.$this->name.'/postdata.php?data='.$this->context->cart->id.'&key='.$key,
            'callback_url' => $protocol_link . Shiptomyid::getShopDomainSsl() . __PS_BASE_URI__ . 'modules/' . $this->name . '/postdata.php?data=' . $this->context->cart->id . '&key=' . $key,
            'marketplace_name' => $marketplace_name,
            'marketplace_image' => $marketplace_image,
            'giftnow' => $giftnow,
            'ship2myid_front' => $this->context->link->getModuleLink('shiptomyid', 'front')
	...
	$order = new Order($shipto_order->id_order);
	$this->sendAdminNotification($shipto_order->id_order, $order->id_shop, $order->id_lang);
    ...
    public function sendAdminNotification($id_order, $id_shop, $id_lang) {
        $template_vars = array(
            '{order_id}' => $id_order,
        );
        $iso = Language::getIsoById($id_lang);
        if (file_exists(dirname(__FILE__) . '/mails/' . $iso . '/admin_notify.txt') &&
	file_exists(dirname(__FILE__) . '/mails/' . $iso . '/admin_notify.html'))
            Mail::Send(
	    $id_lang, 'admin_notify', Mail::l('Ship2MyId Order Accepted By Receiver'), $template_vars, (string) (string) Configuration::get('PS_SHOP_EMAIL', null, null, $id_shop), // Receiver Email
	    null, (string) Configuration::get('PS_SHOP_EMAIL', null, null, $id_shop), // Sender Email
	    (string) Configuration::get('PS_SHOP_NAME', null, null, $id_shop), null, null, dirname(__FILE__) . '/mails/', false, $id_shop
            );
    }
	...
	Configuration::updateValue('GIFTNOW_ENABLE', Tools::getValue('GIFTNOW_ENABLE'));
	Configuration::updateValue('GIFTNOW_POPUP_URL', Tools::getValue('GIFTNOW_POPUP_URL'));
	Configuration::updateValue('GIFTNOW_FOR_CATE', Tools::getValue('GIFTNOW_FOR_CATE'));
	Configuration::updateValue('GIFTNOW_FOR_PROD', Tools::getValue('GIFTNOW_FOR_PROD'));
	Configuration::updateValue('GIFTNOW_FOR_ATTR', Tools::getValue('GIFTNOW_FOR_ATTR'));
	Configuration::updateValue('GIFTNOW_CATRGORIES', Tools::getValue('GIFTNOW_CATRGORIES'));
	Configuration::updateValue('GIFTNOW_PRODUCTS', Tools::getValue('GIFTNOW_PRODUCTS'));
	Configuration::updateValue('GIFTNOW_ATTRIBUTE', Tools::getValue('GIFTNOW_ATTRIBUTE'));
	Configuration::updateValue('GIFTNOW_MSG', Tools::getValue('GIFTNOW_MSG'));
	...
	'input' => array(
	    ...
	    // GIFT NOW Module Enable module
	    array(
	        'type' => 'select',
	        'label' => $this->l('Giftnow Enable'),
	        'name' => 'GIFTNOW_ENABLE',
	        'options' => array(
	            'query' => $enable_values,
	            'id' => 'id',
	    ...
	    array(
	        'type' => 'text',
	        'label' => $this->l('Giftnow Popup URL'),
	        'name' => 'GIFTNOW_POPUP_URL',
	        'size' => 64
	    ),
	    array(
	        'type' => 'select',
	        'label' => $this->l('Giftnow Category'),
	        'name' => 'GIFTNOW_FOR_CATE',
	        'options' => array(
	            'query' => $enable_values,
	            'id' => 'id',
	            'name' => 'name',
	        ),
	        'desc' => $this->l('Select Gift Now For any perticular category.'),
	    ),
	    array(
	        'type' => 'textarea',
	        'label' => $this->l('Giftnow Category'),
	        'name' => 'GIFTNOW_CATRGORIES',
	        'size' => 64,
	        'desc' => $this->l('Add category for Gift Now in Comma seperated like 2,5,7'),
	    ),
	    array(
	        'type' => 'select',
	        'label' => $this->l('Giftnow Products'),
	        'name' => 'GIFTNOW_FOR_PROD',
	        'options' => array(
	            'query' => $enable_values,
	            'id' => 'id',
	            'name' => 'name',
	        ),
	        'desc' => $this->l('Select Gift Now For any perticular Product.'),
	    ),
	    array(
	        'type' => 'textarea',
	        'label' => $this->l('Giftnow Product'),
	        'name' => 'GIFTNOW_PRODUCTS',
	        'size' => 64,
	        'desc' => $this->l('Add Products for Gift Now in Comma seperated like 2,5,7'),
	    ),
	    array(
	        'type' => 'select',
	        'label' => $this->l('Giftnow Arrtibute'),
	        'name' => 'GIFTNOW_FOR_ATTR',
	        'options' => array(
	            'query' => $enable_values,
	            'id' => 'id',
	            'name' => 'name',
	        ),
	        'desc' => $this->l('Select Gift Now For any perticular Attribute.'),
	    ),
	    array(
	        'type' => 'text',
	        'label' => $this->l('Giftnow Arribute'),
	        'name' => 'GIFTNOW_ATTRIBUTE',
	        'size' => 64,
	        'desc' => $this->l('Add Attribute name for Gift Now with ID like giftnow:8'),
	    ),
	    array(
	        'type' => 'textarea',
	        'label' => $this->l('Giftnow Message'),
	        'name' => 'GIFTNOW_MSG',
	        'desc' => $this->l('This Message will showup as Tool Tip on "What is Gift Now"'),
	    ),
	),
	'submit' => array(
	    'title' => $this->l('Save'),
    ...
    private function getConfigFieldsValues($states) {
        return array(
            ...
            'GIFTNOW_ENABLE' => Tools::getValue('GIFTNOW_ENABLE', Configuration::get('GIFTNOW_ENABLE')),
            'GIFTNOW_POPUP_URL' => Tools::getValue('GIFTNOW_POPUP_URL', Configuration::get('GIFTNOW_POPUP_URL')),
            //'GIFTNOW_JS' => Tools::getValue('GIFTNOW_JS', Configuration::get('GIFTNOW_JS')),
            'GIFTNOW_FOR_CATE' => Tools::getValue('GIFTNOW_FOR_CATE', Configuration::get('GIFTNOW_FOR_CATE')),
            'GIFTNOW_FOR_PROD' => Tools::getValue('GIFTNOW_FOR_PROD', Configuration::get('GIFTNOW_FOR_PROD')),
            'GIFTNOW_FOR_ATTR' => Tools::getValue('GIFTNOW_FOR_ATTR', Configuration::get('GIFTNOW_FOR_ATTR')),
            'GIFTNOW_CATRGORIES' => Tools::getValue('GIFTNOW_CATRGORIES', Configuration::get('GIFTNOW_CATRGORIES')),
            'GIFTNOW_PRODUCTS' => Tools::getValue('GIFTNOW_PRODUCTS', Configuration::get('GIFTNOW_PRODUCTS')),
            'GIFTNOW_ATTRIBUTE' => Tools::getValue('GIFTNOW_ATTRIBUTE', Configuration::get('GIFTNOW_ATTRIBUTE')),
            'GIFTNOW_MSG' => Tools::getValue('GIFTNOW_MSG', Configuration::get('GIFTNOW_MSG'))
        );
    }
