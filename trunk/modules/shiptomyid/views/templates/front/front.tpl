...
<script type="text/javascript">
    ...
    function buildAddressBlock(id_address, address_type, dest_comp) {
        ...
        console.log('dest_comp'+dest_comp);
        if (isNaN(id_address))
            return;
            ...
            <div class="col-main">
                ...
                <div class="addresses clearfix">
                    ...
                    <p class="address_delivery select">
                        <input type="hidden" id="id_address_delivery" name="id_address_delivery"
                               value="{$shipto_delivery_address->id|intval}"/>
                        <label for="id_address_invoice"
                               class="strong">{l s='Delivery address : ' mod='shiptomyid'}</label>{$shipto_delivery_address->alias|escape:'htmlall':'UTF-8'}
                    </p>

                    <p id="address_invoice_form" class="select">
                        <label for="id_address_invoice"
                               class="strong">{l s='Choose a billing address:' mod='shiptomyid'}</label>
                        <select name="id_address_invoice" id="id_address_invoice" class="address_select"
                                onchange="updateAddressesDisplay();">
                            {foreach from=$addresses key=k item=address}
                                {if !isset($address.shipto_addr)}
                                    <option value="{$address.id_address|intval}"
                                            {if $address.id_address == $cart->id_address_invoice}selected="selected"{/if}>{$address.alias|escape:'htmlall':'UTF-8'}</option>
                                {/if}
                            {/foreach}
                        </select>
                    </p>
                ...
                <p class="cart_navigation submit">
                    ...
                    <a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if isset($back) && $back}&back={$back}{/if}")|escape:'htmlall':'UTF-8'}"
                       title="{l s='Previous'  mod='shiptomyid'}"
                       class="button">&laquo; {l s='Previous'  mod='shiptomyid'}</a>
                    <input type="submit" name="processAddress" value="{l s='Next' mod='shiptomyid'} &raquo;"
                           class="exclusive"/>
                </p>
