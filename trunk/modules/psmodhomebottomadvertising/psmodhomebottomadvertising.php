...
include_once(dirname(__FILE__) . '/psmodhomebottomadvertisingModel.php');
include_once(dirname(__FILE__) . '/psmodhomebottomadvertising_class.php');

class PsModHomeBottomAdvertising extends Module {

    private $_html;
    private $_display;

    public function __construct() {
        $this->name = 'psmodhomebottomadvertising';
        $this->tab = 'front_office_features';
        $this->version = '1.1.0';
        $this->author = 'Any-Themes';
        $this->need_instance = 0;
        parent::__construct();
        $this->displayName = $this->l('Any-Themes Home Bottom Advertising');
        $this->description = $this->l('Any-Themes Advertising on Home page bottom .');
    ...
    public function install() {
        ...
        if (parent::install() && $this->registerHook('home') && $this->registerHook('header')) {
            /* Install tables */
            $res = psmodhomebottomadvertisingModel::createTables();
            return $res;
        }
        return false;
    }
    ...
    public function uninstall() {
        ...
        if (parent::uninstall()) {
            /* Deletes tables */
            $res = psmodhomebottomadvertisingModel::DropTables();
            return $res;
        }
        return false;
    }
    ...
    public function getContent() {
        ...
        elseif (Tools::isSubmit('addNewSlider') || (Tools::isSubmit('id_psmodhomebottomadvertising')))
            $this->_displayForm();
        else
            $this->_displaySlider();
    ...
    private function _insertSlider() {
        ...
        if (Tools::isSubmit('sccSubmitValue')) {
			...
			SELECT COUNT(*) 
			FROM `' . _DB_PREFIX_ . 'psmodhomebottomadvertising`');




                Db::getInstance()->Execute('
            INSERT INTO `' . _DB_PREFIX_ . 'psmodhomebottomadvertising` (`image_link`,`active`,`position`,`new_page`) 
            VALUES("' . Tools::getValue('scc_link') . '", ' . (int) Tools::getValue('scc_active_slide') . ',' . (int) $position . ',' . (int) Tools::getValue('scc_new_page') . ')');

                $id_psmodhomebottomadvertising = Db::getInstance()->Insert_ID();
                foreach ($languages as $language) {
                    $name = $_FILES['scc_image_' . $language['id_lang']]['name'];
                    $image_url = PSMOD_HOME_LEFT_ADD_IMAGE_URL . $name;

                    $path = PSMOD_HOME_LEFT_ADD_MODULE_IMAGE_DIR . $name;
                ...
                VALUES(' . (int) $id_psmodhomebottomadvertising . ', ' . (int) $language['id_lang'] . ', 
                "' . pSQL(Tools::getValue('scc_title_' . $language['id_lang'])) . '", 
                 "' . $image_url . '")');
                }
            } elseif (Tools::isSubmit('updateSlider')) {

                $sccsliderid = Tools::getvalue('id_psmodhomebottomadvertising');
                ...
                WHERE `id_psmodhomebottomadvertising` = ' . (int) ($sccsliderid));
                ...
                foreach ($languages as $language) {
                    ...
                    Db::getInstance()->Execute('
                            UPDATE `' . _DB_PREFIX_ . 'psmodhomebottomadvertising_lang` 
                            SET `image_title` = "' . pSQL(Tools::getValue('scc_title_' . $language['id_lang'])) . '",                    
                            `image_url` = "' . $image_url . '"
                            WHERE `id_psmodhomebottomadvertising` = ' . (int) $sccsliderid . '  AND `id_lang`= ' . (int) $language['id_lang']);
                }
            }
        }
        elseif (Tools::isSubmit('deleteSlider') AND Tools::getValue('id_psmodhomebottomadvertising')) {
            Db::getInstance()->Execute('
                DELETE FROM `' . _DB_PREFIX_ . 'psmodhomebottomadvertising`
                WHERE `id_psmodhomebottomadvertising` = ' . (int) (Tools::getValue('id_psmodhomebottomadvertising')));

            Db::getInstance()->Execute('
				DELETE FROM `' . _DB_PREFIX_ . 'psmodhomebottomadvertising_lang` 
				WHERE `id_psmodhomebottomadvertising` = ' . (int) (Tools::getValue('id_psmodhomebottomadvertising')));
        }
        if (count($errors))
    ...
    private function _postValidation() {
        ...
        elseif (Tools::isSubmit('deleteSlider') AND !Validate::isInt(Tools::getValue('id_psmodhomebottomadvertising')))
    ...
    private function _displayForm() {
        ...
        $updatevalue = NULL;
        if (Tools::isSubmit('updateSlider') AND Tools::getValue('id_psmodhomebottomadvertising'))
            $updatevalue = psmodhomebottomadvertisingModel::getSliderByID((int) Tools::getValue('id_psmodhomebottomadvertising'));
//print_r($updatevalue);
        ...
        $this->_html .= '
		<fieldset>
			<legend><img src="' . _PS_BASE_URL_ . __PS_BASE_URI__ . 'modules/' . $this->name . '/logo.gif" alt="" /> ' . $this->l(' Home Advertising') . '</legend>
			';

        $this->_html.= '<form method="post" action="' . Tools::safeOutput($_SERVER['REQUEST_URI']) . '" enctype="multipart/form-data">
      <fieldset>
        <legend>' . $this->l('Add A New Advertise') . '</legend>';
...
';
    ...
    private function _displaySlider() {
        ...
        $slider = psmodhomebottomadvertisingModel::getAllSlider();
...
<script type="text/javascript">SCCImageSlider(\'' . $this->secure_key . '\');</script>';
        ...
        $this->_html .= '<p><a href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&addNewSlider"><img src="' . _PS_ADMIN_IMG_ . 'add.gif" alt="" /> ' . $this->l('Add a new Advertising') . '</a></p>';
        if ($slider):
        ...
        foreach ($slider as $sccsliderdata):
            ...
            $this->_html .= '<tr id="tr_0_' . $sccsliderdata['id_psmodhomebottomadvertising'] . '_' . $sccsliderdata['position'] . '" ' . ($irow++ % 2 ? 'class="alt_row"' : '') . '>
                             <td>' . $sccsliderdata['id_psmodhomebottomadvertising'] . '</td>
            ...
            $this->_html .= '</td> 
                            ...
                            <td class="center pointer dragHandle">
                                    <a' . (($sccsliderdata['position'] == (sizeof($sccsliderdata) - 1) OR sizeof($sccsliderdata) == 1) ? ' style="display: none;"' : '') . ' href="' . $currentIndex . '&configure=psmodhomebottomadvertising&id_psmodhomebottomadvertising=' . $sccsliderdata['id_psmodhomebottomadvertising'] . '&token=' . Tools::getAdminTokenLite('AdminModules') . '">
                                    <img src="../img/admin/down.gif" alt="' . $this->l('Down') . '" title="' . $this->l('Down') . '" /></a>
                                    <a' . ($sccsliderdata['position'] == 0 ? ' style="display: none;"' : '') . ' href="' . $currentIndex . '&configure=psmodhomebottomadvertising&id_psmodhomebottomadvertising=' . $sccsliderdata['id_psmodhomebottomadvertising'] . '&token=' . Tools::getAdminTokenLite('AdminModules') . '">
                                    <img src="../img/admin/up.gif" alt="' . $this->l('Up') . '" title="' . $this->l('Up') . '" /></a>
                            </td>
                              <td width="10%" class="center">
                            
                                        <a href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&updateSlider&id_psmodhomebottomadvertising=' . (int) ($sccsliderdata['id_psmodhomebottomadvertising']) . '" title="' . $this->l('Edit') . '"><img src="' . _PS_ADMIN_IMG_ . 'edit.gif" alt="" /></a> 
                                        <a href="' . $currentIndex . '&configure=' . $this->name . '&token=' . Tools::getAdminTokenLite('AdminModules') . '&deleteSlider&id_psmodhomebottomadvertising=' . (int) ($sccsliderdata['id_psmodhomebottomadvertising']) . '" title="' . $this->l('Delete') . '"><img src="' . _PS_ADMIN_IMG_ . 'delete.gif" alt="" /></a>
    ...
    function hookHome($params) {
        global $smarty;
            ...
            SELECT scc.`id_psmodhomebottomadvertising`, scc.`image_link`, scc.`active`, scc.`position`, scc.`new_page`,scc1.image_title, scc1.`image_url`
            ...
            return $this->display(__FILE__, 'psmodhomebottomadvertising.tpl');
        }
