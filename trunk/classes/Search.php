...
class SearchCore
{
		...
		$order_way = 'desc', $ajax = false, $use_cookie = true, Context $context = null)
	{
		...
		$sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, 
			 ...
			 MAX(image_shop.`id_image`) id_image, il.`legend`, m.`name` manufacturer_name '.$score.', AVG(pc.`grade`) as rating, Count(DISTINCT(pc.`id_product_comment`)) as reviews, MAX(product_attribute_shop.`id_product_attribute`) id_product_attribute,
				DATEDIFF(
					p.`date_add`,
					DATE_SUB(
						NOW(),
