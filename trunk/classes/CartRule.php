...
class CartRuleCore extends ObjectModel
{
    ...
    public function checkValidity(Context $context, $alreadyInCart = false, $display_error = true)
    {
        ...
        if ($this->minimum_amount) {
            ...
            //adding custom code for currency conversion issue
            if ($context->cart->id_currency != Configuration::get('PS_CURRENCY_DEFAULT')) {
                $cart_total_currency_conversion = new Currency($context->cart->id_currency);
                $cartTotal = $cartTotal / $cart_total_currency_conversion->conversion_rate;
            }

            //end custom code for vouchers currency conversion issue
