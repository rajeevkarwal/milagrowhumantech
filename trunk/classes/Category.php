...
class CategoryCore extends ObjectModel
{
    ...
    public function getProducts($id_lang, $p, $n, $order_by = null, $order_way = null, $get_total = false, $active = true, $random = false, $random_number_products = 1, $check_access = true, Context $context = null)
    {
        ...
        $sql = 'SELECT p.*, product_shop.*, stock.out_of_stock, IFNULL(stock.quantity, 0) as quantity, MAX(product_attribute_shop.id_product_attribute) id_product_attribute, product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity, pl.`description`, pl.`description_short`, pl.`available_now`,
					...
					il.`legend`, m.`name` AS manufacturer_name, AVG(pc.`grade`) as rating, cl.`name` AS category_default,
            ...
            Shop::addSqlAssociation('image', 'i', false, 'image_shop.cover=1') . '
				...
				LEFT JOIN `' . _DB_PREFIX_ . 'manufacturer` m
					ON m.`id_manufacturer` = p.`id_manufacturer` LEFT JOIN `' . _DB_PREFIX_ . 'product_comment` pc ON pc.`id_product`=p.`id_product`
