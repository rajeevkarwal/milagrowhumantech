...
class FrontControllerCore extends Controller
{
	...
	public function initContent()
	{
        ...
        $catIds=array();
                $categoriesHtml = Db::getInstance()->ExecuteS('SELECT cb.content_text,cb.content_id,cb.id_category,pcl.name FROM ' . _DB_PREFIX_ . 'contentbox cb left join ps_category_lang pcl on pcl.id_category=cb.id_category left join ps_category pc on pcl.id_category=pc.id_category');
                foreach($categoriesHtml as $row)
                {
                    if($row['id_category']!=0)
                    $catIds[]=$row['id_category'];
                }

	$catWiseHtml=array();
	if(!empty($catIds))
	{
		$childQuery='SELECT id_category,id_parent FROM ' . _DB_PREFIX_ . 'category where id_parent in ('.implode(',',$catIds).')';
        $childCategories = Db::getInstance()->ExecuteS($childQuery);

        $parentWiseChilds=array();
        foreach($childCategories as $row)
        {
           $parentWiseChilds[$row['id_parent']][]=$row['id_category'];
        }

        
        foreach($categoriesHtml as $rowItem)
        {
            $catWiseHtml[$rowItem['id_category']]=$rowItem['content_text'];
            if(isset($parentWiseChilds[$rowItem['id_category']]))
            {
                foreach($parentWiseChilds[$rowItem['id_category']] as $row2)
                {
                    $catWiseHtml[$row2]=$rowItem['content_text'];
                }
            }
        }
	}	
        ...
        $this->context->smarty->assign(array('catHtml'=>$catWiseHtml));
	}
...
//        exit;
	...
	public function productSort()
	{
		...
		$order_by_values = array(0 => 'name', 1 => 'price', 2 => 'date_add', 3 => 'date_upd', 4 => 'position', 5 => 'manufacturer_name', 6 => 'quantity',7=>'rating');
