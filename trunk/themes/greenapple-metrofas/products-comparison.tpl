                            ...
                            <div class="products_block">
                                ...
                                <table class="data-table cart-table" id="shopping-cart-table">
                                    ...
                                    {foreach from=$products item=product name=for_products}
                                        ...
                                        <td width="{$width}%" class="ajax_block_product comparison_infos align-center">
                                            <a href="{$product->getLink()}" title="{$product->name|escape:html:'UTF-8'}" class="product_image" >
                                                <img src="{$link->getImageLink($product->link_rewrite, $product->id_image, 'home_default')}" alt="{$product->name|escape:html:'UTF-8'}" width="120" height="120" />
                                            </a>
                                            <h5 class="align-center"><a href="{$product->getLink()}" title="{$product->name|truncate:32:'...'|escape:'htmlall':'UTF-8'}">{$product->name|truncate:27:'...'|escape:'htmlall':'UTF-8'}</a></h5>
                                            <div class="product_desc align-center"><a href="{$product->getLink()}">{$product->description_short|strip_tags|truncate:60:'...'}</a></div>
                                            ...
                                            <div class="comparison_product_infos">
                                                    ...
                                                    {if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE}
                                                        ...
                                                        <div class="product_discount">
                                                            ...
                                                            {elseif $product->specificPrice AND $product->specificPrice.reduction}
                                                                <span class="">{l s='Reduced price!'}</span>
                                                            {/if}
                                                        </div>
                                                ...
                                                {if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
                                                    {if ($product->quantity > 0 OR $product->allow_oosp)}
                                                        <a class="ajax_add_to_cart_button button btn-cart f-left" rel="ajax_id_product_{$product->id}"  title="{l s='Add to cart'}"><span><span>{l s='Add to cart'}</span></span></a>
                                                    {else}
                                                        <a class="button btn-cart f-left"  title="{l s='Out fo stock'}"><span><span>{l s='Out fo stock'}</span></span></a>
