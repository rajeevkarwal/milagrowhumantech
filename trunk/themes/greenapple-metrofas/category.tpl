        ...
        <div class="{if $themesdev.td_siderbar_without=="enable"}col-main span9{else}std{/if}">
            ...
            {if isset($category)}
                {if $category->id AND $category->active}
                    ...
                    {if $scenes || $category->description || $category->id_image}
                        <p class="category-image">
                            ...
                            {else}
                                ...
                                {*<img src="{$link->getCatImageLink($category->link_rewrite, $category->id_image)}"*}
                                {*alt="{$category->name|escape:'htmlall':'UTF-8'}"*}
                                {*title="{$category->name|escape:'htmlall':'UTF-8'}"/>*}
                                    <img src="/img/c/original/category_original{$category->id_image}.jpg"
                                         alt="{$category->name|escape:'htmlall':'UTF-8'}"
                                         title="{$category->name|escape:'htmlall':'UTF-8'}"/>
                                {/if}
                            {/if}
                    ...
                    {if ($themesdev.td_showsubcat=='enable')}
                                            ...
                                            <a href="{$link->getCategoryLink($subcategory.id_category, $subcategory.link_rewrite)|escape:'htmlall':'UTF-8'}"
                                               title="{$subcategory.name|escape:'htmlall':'UTF-8'}" class="img">
                                                {if $subcategory.id_image}
                                                    <img src="{$link->getCatImageLink($subcategory.link_rewrite, $subcategory.id_image, 'medium_default')}"
                                                         alt="" width="{$mediumSize.width}"
