        ...
        <div class="col-main">
            <div class="account-login">
                ...
                <div class="page-title">
                    {if !$opc}<h1>{l s='Addresses'}</h1>{else}
                    ...
                    {if {Configuration::get('PS_ALLOW_MULTISHIPPING')} && !$cart->isVirtualCart()}
                        <div class="address-form-multishipping">
                            <div class="button_multishipping_mode" id="multishipping_mode_box">
                                <div class="title">{l s='Multi-shipping'}</div>
                                <div class="description">
                                    <input type="checkbox" id="multishipping_mode_checkbox"
                                           onchange="multishippingMode(this); return false;"/><label
                                            for="multishipping_mode_checkbox">{l s='I\'d like to specify a delivery address for each product ordered.'}</label>
                                </div>
                                <div class="description_off">
                                    <a href="{$link->getPageLink('order-opc', true, NULL, 'ajax=1&multi-shipping=1&method=multishipping')}"
                                       id="link_multishipping_form" title="{l s='Choose the delivery address(es)'}">
                    ...
                    <div id="opc_account" class="opc-main-block">
                        ...
                        <div class="addresses clearfix">
                            <p class="address_delivery select">
                                ...
                                <img src="{$img_dir}ajax-loader-address-update.gif" id="update-address-loader"
                                     style="display:none;padding:6px;"/>
                            </p>
                            <p class="checkbox addressesAreEquals"
                               {if $cart->isVirtualCart()}style="display:none;"{/if}>
                            ...
                            <p id="address_invoice_form" class="select"
                               {if $cart->id_address_invoice == $cart->id_address_delivery}style="display: none;"{/if}>

                                {if $addresses|@count > 1}
                                    <label for="id_address_invoice"
                                           class="strong">{l s='Choose a billing address:'}</label>
                                    <select name="id_address_invoice" id="id_address_invoice" class="address_select"
                                        ...
                                        {section loop=$addresses step=-1 name=address}
                                            <option value="{$addresses[address].id_address|intval}"
                                                    {if $addresses[address].id_address == $cart->id_address_invoice && $cart->id_address_delivery != $cart->id_address_invoice}selected="selected"{/if}>{$addresses[address].alias|escape:'htmlall':'UTF-8'}</option>
                                        {/section}
                                    </select>
                                {else}
                                    <a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1&select_address=1{if $back}&mod={$back}{/if}")}"
                                       title="{l s='Add'}" class="button">{l s='Add a new address'}</a>
                                {/if}
                            </p>
                            <div class="clearfix">
                                ...
                                <ul class="address alternate_item {if $cart->isVirtualCart()}full_width{/if}"
                                    id="address_invoice">
                                </ul>
                            </div>
                            <p class="address_add submit">
                                <a href="{$link->getPageLink('address', true, NULL, "back={$back_order_page}?step=1{if $back}&mod={$back}{/if}")}"
                                   title="{l s='Add'}" class="button">{l s='Add a new address'}</a>
                            </p>
                            {if !$opc}
                                <div id="ordermsg" class="clearfix">
                                    <p class="txt">{l s='If you would like to add a comment about your order, please write it in the box.'}</p>

                                    <p class="textarea"><textarea rows="3" placeholder="Write your comment here"
                                                                  name="message">{if isset($oldMessage)}{$oldMessage}{/if}</textarea>
                                    </p>
                                </div>
                            {/if}
                        </div>
                        ...
                        <p class="cart_navigation submit">
                            ...
                            <a href="{$link->getPageLink($back_order_page, true, NULL, "step=0{if $back}&back={$back}{/if}")}"
                               title="{l s='Previous'}" class="button left"> <span><span>&laquo; {l s='Previous'}</span></span></a>
                            <button type="submit" name="processAddress" class="button right">
                                <span><span>{l s='Next'} &raquo;</span></span></button>
                        </p>
                </form>
                {else}
            </div>
