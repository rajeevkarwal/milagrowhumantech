                    ...
                    <div class="{if $themesdev.td_siderbar_without=="enable"}col-main span9{else}std{/if}">
                        ...
                        {else}
                            ...
                            <form action="{$request_uri|escape:'htmlall':'UTF-8'}" method="post" class="std" enctype="multipart/form-data">
                                <fieldset>
                                    <p class="bold">{l s='If you have bought the product before 15'}<sup>th</sup> of October, 2013 and/or other website than simply right a mail at customercare@milagrow.in or call us at 09953476189 </p>
                                    <p class="select">
                                                    ...
                                                    {foreach from=$orderedProductList key=id_order item=products name=products}
                                                        <select name="id_product" id="{$id_order}_order_products" class="product_select" style="{if !$smarty.foreach.products.first} display:none; {/if}" {if !$smarty.foreach.products.first}disabled="disabled" {/if}>
                                                            ...
                                                            {foreach from=$products item=product}
                                                                <option value="{$product.value|intval}">{$product.label|escape:'htmlall':'UTF-8'|truncate:50:"..."}</option>
                                                            {/foreach}
                                                        </select>
                                                    {/foreach}
                                    ...
                                    <p class="textarea">
                                        ...
                                        <textarea id="message" name="message" rows="15" cols="10" placeholder="Please mention your order number and date of purchase in case of lack of order reference.">{if isset($message)}{$message|escape:'htmlall':'UTF-8'|stripslashes}{/if}</textarea>
                                    </p>
                                    <p class="submit">
                                        <button type="submit" name="submitMessage" id="submitMessage"  class="button" onclick="$(this).hide();" ><span><span>{l s='Send'}</span></span></button>
                                    </p>
                                </fieldset>
                            </form>
                        {/if}
