                    ...
                    <div class="account-login">
                        ...
                        {else}
                            ...
                            <form action="{$link->getModuleLink('bankwire', 'validation', [], true)}" method="post">
                                <p>
                                    <img src="{$this_path}bankwire.jpg" alt="{l s='bank wire' mod='bankwire'}"
                                        ...
                                        <select id="currency_payement" name="currency_payement"
                                            ...
                                            {foreach from=$currencies item=currency}
                                                <option value="{$currency.id_currency}"
                                                        {if $currency.id_currency == $cust_currency}selected="selected"{/if}>{$currency.name}</option>
                                            {/foreach}
                                        </select>
                                    {else}
                                        {l s='We accept the following currency to be sent by bank wire:' mod='bankwire'}&nbsp;
                                        <b>{$currencies.0.name}</b>
                                        <input type="hidden" name="currency_payement"
                                               value="{$currencies.0.id_currency}"/>
                                    {/if}
                                </p>
                                <p>
                                    {l s='Bank wire account information will be displayed on the next page.' mod='bankwire'}
                                    <br/><br/>
                                    <b>{l s='Please confirm your order by clicking \'I confirm my order\'' mod='bankwire'}
                                        .</b>
                                </p>

                                <p class="cart_navigation">
                                    <button type="submit" name="submit" class="button">
                                        <span><span>{l s='I confirm my order' mod='bankwire'}</span></span></button>

                                    <button onclick="window.location='{$link->getPageLink('order', true, NULL, "step=3")}'"
                                            title="{l s='Other payment methods' mod='bankwire'}" class="button right"
                                            type="button">
                                        <span><span>{l s='Other payment methods' mod='bankwire'}</span></span></button>
