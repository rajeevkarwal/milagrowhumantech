                ...
                <div id="view_wishlist">
                    ...
                    {if $products}
                        <div class="addresses" id="featured-products_block_center">
                            <h3>{l s='Welcome to the wishlist of' mod='blockwishlist'} {$current_wishlist.firstname} {$current_wishlist.lastname}</h3>

                            <p/>
                            {foreach from=$products item=product name=i}
                                <ul class="address {if $smarty.foreach.i.last}last_item{elseif $smarty.foreach.i.first}first_item{/if} {if $smarty.foreach.i.index % 2}alternate_item{else}item{/if}"
                                    id="block_{$product.id_product}_{$product.id_product_attribute}">
                                    <div class="ajax_block_product">
                                        <li class="address_title"><a href="{$link->getProductLink($product.id_product,
                                            $product.link_rewrite, $product.category_rewrite)}"
                                                                     title="{l s='View' mod='blockwishlist'}">{$product.name|truncate:30:'...'|escape:'htmlall':'UTF-8'}</a>
                                        </li>
                                        <li class="address_name">
                                            <a href="{$link->getProductlink($product.id_product, $product.link_rewrite, $product.category_rewrite)}"
                                               title="{l s='Product detail' mod='blockwishlist'}" class="product_image">
                                                <img src="{$link->getImageLink($product.link_rewrite, $product.cover, 'medium_default')}"
                                            ...
                                            {*{/if}*}
                                            {*<br />{l s='Quantity:' mod='blockwishlist'}<input type="text" id="{$product.id_product}_{$product.id_product_attribute}" size="3" style="width:50px !important;" value="{$product.quantity|intval}" readonly/>*}
                                        ...
                                        <li class="address_address1 clear">
                                            ...
                                            {*</form>*}
                                            {*{/if}*}
                                            {*<a href="javascript:;" class="exclusive" onclick="WishlistBuyProduct('{$token|escape:'htmlall':'UTF-8'}', '{$product.id_product}', '{$product.id_product_attribute}', '{$product.id_product}_{$product.id_product_attribute}', this, {$ajax});" title="{l s='Add to cart' mod='blockwishlist'}">{l s='Add to cart' mod='blockwishlist'}</a>*}
                                            {*{else}*}
                        ...
                        <div id="messages_product_view">
                            <ul class="messages">
                                <li class="note-msg">
                                    <ul>
                                        <li>
                                            <span>{l s='No products' mod='blockwishlist'}</span></li>
                                    </ul>
                                </li>
                            </ul>
                        </div>
