                            ...
                            <div class="addresses clearfix">
                                ...
                                <p id="address_invoice_form" class="select" {if $cart->id_address_invoice == $cart->id_address_delivery}style="display: none;"{/if}>
                                    ...
                                    {if $addresses|@count >= 1}
                                        ...
                                        <img src="{$img_dir}ajax-loader-address-update.gif" id="update-address-loader" style="display:none;padding:6px;"/>
                                    {else}
                                ...
                                {if !$opc}
                                    <div id="ordermsg" class="clearfix">
                                        ...
                                        <p class="textarea"><textarea cols="60" rows="3" placeholder="Write your comment here" name="message">{if isset($oldMessage)}{$oldMessage}{/if}</textarea></p>
                                    </div>
                                {/if}
                            </div>
                            {if !$opc}
                                <p class="cart_navigation submit">
                                    ...
                                    {if $back}
                                        <button onclick="window.location='{$link->getPageLink('order', true, NULL, "step=0&amp;back={$back}")}'"  title="{l s='Previous'}" class="button left" type="button"><span><span>&laquo; {l s='Previous'}</span></span></button>

                                    {else}
                                        <button onclick="window.location='{$link->getPageLink('order', true, NULL, "step=0")}'"  title="{l s='Previous'}" class="button left" type="button"><span><span>&laquo; {l s='Previous'}</span></span></button>
                                    {/if}
                                    <button type="submit" name="processAddress" class="button right" ><span><span>{l s='Next'} &raquo;</span></span></button>
                                </p>
                        </form>
                    {else}
                    </div>
