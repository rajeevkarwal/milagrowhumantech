...
function reloadContent(params_plus)
{
	...
	if (!ajaxLoaderOn)
	{
		$('#products_wrapper').prepend('<div class="loading">'+$('#layered_ajax_loader').html()+'</div>');
		$('#products-list').css('opacity', '0.7');
                $('#grid_view_product').css('opacity', '0.7');
	...
	ajaxQuery = $.ajax(
	{
		...
		success: function(result)
		{
			...
			if (result.productList){
                            var is_grid = $('#grid_view_product').is(':visible');
                            var is_list = $('#products-list').is(':visible');
                            
                            $('#products_wrapper').replaceWith(utf8_decode(result.productList));
                            if(is_grid){
                                $('#grid_view_product').show();
                                $('#products-list').hide();
                            } else if(is_list){
                                $('#grid_view_product').hide();
                                $('#products-list').show();
                            }
                        } else{
                            $('#products-list').html('');
                            $('#grid_view_product').html('');
                        }

			$('#products-list').css('opacity', '1');
                        $('#grid_view_product').css('opacity', '1');
			if ($.browser.msie){ // Fix bug with IE8 and aliasing
                            $('#products-list').css('filter', '');
                            $('#grid_view_product').css('filter', '');
                        }
...
function updateProductUrl()
{
	...
	if (typeof(param_product_url) != 'undefined') {
		$.each($('ol#products-list li.ajax_block_product .product_img_link,'+
                            'ol#products-list li.ajax_block_product h3 a,'+
                            'ol#products-list li.ajax_block_product .product_desc a,'+
                            'ol#products-list li.ajax_block_product .lnk_view'+
                        'ul#grid_view_product li.ajax_block_product .product_img_link,'+
                            'ul#grid_view_product li.ajax_block_product h3 a,'+
                            'ul#grid_view_product li.ajax_block_product .product_desc a,'+
                            'ul#grid_view_product li.ajax_block_product .lnk_view'), function() {
