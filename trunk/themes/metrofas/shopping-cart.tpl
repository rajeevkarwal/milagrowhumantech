                    ...
                    <li>   
                        {if !$opc}
                            ...
                            {if Configuration::get('PS_ALLOW_MULTISHIPPING')}
                                <button onclick="window.location='{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}&amp;multi-shipping=1';" class="button btn-proceed-checkout btn-checkout" title="{l s='Proceed to Checkout'}" type="button"><span><span>{l s='Proceed to Checkout'}</span></span></button>
                            {else}
                                <button onclick="window.location='{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}';" class="button btn-proceed-checkout btn-checkout" title="{l s='Proceed to Checkout'}" type="button"><span><span>{l s='Proceed to Checkout'}</span></span></button>
                            {/if}
                            ...
                            <tr class="first">
                                <td colspan="50" class="a-right">
                                    <button type="button" title="Continue Shopping" class="button btn-continue" 
                                            onclick="window.location=('{if (isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order.php')) || isset($smarty.server.HTTP_REFERER) && strstr($smarty.server.HTTP_REFERER, 'order-opc') || !isset($smarty.server.HTTP_REFERER)}{$link->getPageLink('index')}{else}{$smarty.server.HTTP_REFERER|escape:'htmlall':'UTF-8'|secureReferrer}{/if}')" title="{l s='Continue shopping'}">
                                        <span><span>{l s='Continue shopping'}</span></span>
                                    </button>
                                    {if !$opc}
                                        {if Configuration::get('PS_ALLOW_MULTISHIPPING')}
                                            <button  onclick="window.location=('{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}&amp;multi-shipping=1')" type="button" title="{l s='Next'}" class="button btn-update">
                                                <span>
                                                    <span>{l s='Next'}</span>
                                                </span>
                                            </button>
                                        {else}
                                            <button type="submit" onclick="window.location=('{if $back}{$link->getPageLink('order', true, NULL, 'step=1&amp;back={$back}')}{else}{$link->getPageLink('order', true, NULL, 'step=1')}{/if}')" title="{l s='Update Shopping Cart'}" class="button btn-update">
                                                <span>
                                                    <span>{l s='Next'}</span>
                                                </span>
                                            </button>
