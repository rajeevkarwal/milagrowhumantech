...
class AdminCategoriesControllerCore extends AdminController
{
	...
	{
		 //custom code to save original image
        if(($id_category = (int)Tools::getValue('id_category')) &&
            isset($_FILES) && count($_FILES) && $_FILES['image']['name'] != null)
        {
        $filename = 'category_original'.$id_category.'.jpg';
        if(file_exists(_PS_CAT_IMG_DIR_.'original/'.$filename))
            unlink(_PS_CAT_IMG_DIR_.'original/'.$filename);
        if (isset($filename) && copy($_FILES['image']['tmp_name'], _PS_CAT_IMG_DIR_.'original/'.$filename))
            $file_name = $filename;
        }
