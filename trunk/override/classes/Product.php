...
class ProductCore extends ObjectModel
{
    ...
    public static $definition = array(
        ...
        'fields' => array(
            ...
            'condition' => array('type' => self::TYPE_STRING, 'shop' => true, 'validate' => 'isGenericName', 'values' => array('new', 'used', 'refurbished','combo'), 'default' => 'new'),
                                       ...
                                       $only_active = false, Context $context = null)
    {
        ...
        $sql = 'SELECT p.*, product_shop.*, pl.* , m.`name` AS manufacturer_name, s.`name` AS supplier_name,
                        AVG(pc.`grade`) as rating, Count(DISTINCT(pc.`id_product_comment`)) as reviews
            ...
            ($id_category ? 'LEFT JOIN `' . _DB_PREFIX_ . 'category_product` c ON (c.`id_product` = p.`id_product`)' : '') . '
				WHERE pl.`id_lang` = ' . (int)$id_lang . ' AND (pc.`deleted` = 0 OR pc.`deleted`is NULL) ' .
...
AVG(pc.`grade`) as rating, Count(DISTINCT(pc.`id_product_comment`)) as reviews,
        ...
        $sql->leftJoin('product_comment', 'pc', 'pc.`id_product` = p.`id_product`');

        $sql->where('product_shop.`active` = 1');
        $sql->where('pc.`deleted` = 0 OR pc.`deleted` is NULL');
        ...
        return Product::filterProductForOutOfStock(Product::getProductsProperties((int)$id_lang, $result));
    }
                                         ...
                                         $order_by = null, $order_way = null, $beginning = false, $ending = false, Context $context = null)
    {
        ...
        return Product::filterProductForOutOfStock(Product::getProductsProperties($id_lang, $result));
    }
    ...
    public static function sqlStock($product_alias, $product_attribute = 0, $inner_join = false, Shop $shop = null)
    {
        ...
        if (!is_null($product_attribute)) {
            ...
            elseif (is_numeric($product_attribute))
                $sql .= ' AND stock.id_product_attribute = ' . $product_attribute; elseif (is_string($product_attribute))
