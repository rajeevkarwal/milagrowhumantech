    ...
    public function initToolbar()
    {
        if ($this->display == 'view') {
            ...
            elseif ($order->hasBeenPaid())
                $type = $this->l('Standard refund'); else
        ...
        if (Tools::isSubmit('submitShippingNumber') && isset($order)) {
            if ($this->tabAccess['edit'] === '1') {
                ...
                elseif (!Validate::isTrackingNumber(Tools::getValue('tracking_number')))
                    $this->errors[] = Tools::displayError('The tracking number is incorrect.'); else {
                    // update shipping number
                    ...
                    if ($order_carrier->update()) {
                        ...
                        //custom changes as to stop sending transit mail just send mail when order status changes to shipped.
        ...
        elseif (Tools::isSubmit('submitState') && isset($order)) {
            if ($this->tabAccess['edit'] === '1') {
                ...
                if ($order_state->id == Configuration::get('PS_OS_SHIPPING') && !$order->shipping_number)
                    $this->errors[] = Tools::displayError('Please enter tracking number before changing order status to shipped');
                else {
                        ...
                        if ($current_order_state->id != $order_state->id) {
                            ...
                            if ($history->id_order_state == Configuration::get('PS_OS_SHIPPING') && $order->shipping_number) {
                                ...
                                if ($row2 = Db::getInstance()->getRow($query)) {
                                    ...
                                    $trackingText = 'You can track your package using the following link: <br/> <a href="' . $url . '">' . $url . '</a>';
                                }
                                $templateVars = array('{followup}' => $trackingText);
                            }
                            // Save all changes
        ...
        elseif (Tools::isSubmit('submitMessage') && isset($order)) {
            if ($this->tabAccess['edit'] === '1') {
                ...
                elseif (!Tools::getValue('message'))
                    $this->errors[] = Tools::displayError('The message cannot be blank.'); else {
                    ...
                    if (!count($this->errors)) {
                        ...
                        elseif ($customer_message->private)
                            Tools::redirectAdmin(self::$currentIndex . '&id_order=' . (int)$order->id . '&vieworder&conf=11&token=' . $this->token); else {
        ...
        elseif (Tools::isSubmit('cancelProduct') && isset($order)) {
            if ($this->tabAccess['delete'] === '1') {
                ...
                elseif (!Tools::isSubmit('cancelQuantity') && !Tools::isSubmit('cancelCustomizationQuantity'))
                    $this->errors[] = Tools::displayError('You must enter a quantity.'); else {
            ...
            Message::markAsReaded(Tools::getValue('messageReaded'), $this->context->employee->id); elseif (Tools::isSubmit('submitAddPayment') && isset($order)) {
            if ($this->tabAccess['edit'] === '1') {
                ...
                elseif (!Validate::isNegativePrice($amount))
                    $this->errors[] = Tools::displayError('The amount is invalid.'); elseif (!Validate::isString(Tools::getValue('payment_method')))
                    $this->errors[] = Tools::displayError('The selected payment method is invalid.'); elseif (!Validate::isString(Tools::getValue('payment_transaction_id')))
                    $this->errors[] = Tools::displayError('The transaction ID is invalid.'); elseif (!Validate::isLoadedObject($currency))
                    $this->errors[] = Tools::displayError('The selected currency is invalid.'); elseif ($order_has_invoice && !Validate::isLoadedObject($order_invoice))
                    $this->errors[] = Tools::displayError('The invoice is invalid.'); elseif (!Validate::isDate(Tools::getValue('payment_date')))
                    $this->errors[] = Tools::displayError('The date is invalid'); else {
            ...
            elseif ($order->hasInvoice())
                $this->errors[] = Tools::displayError('This order already has an invoice.'); else {
                ...
                if (!Tools::getValue('discount_name'))
                    ...
                    foreach ($cart_rules as &$cart_rule) {
                        ...
                        elseif (Tools::getValue('discount_type') == 2)
                            $cartRuleObj->reduction_amount = $cart_rule['value_tax_excl']; elseif (Tools::getValue('discount_type') == 3)
